...
void test(boolean a, boolean b, int *p, volatile int *q, int i) {
  ...
  b = foo() & (*q == 42);      // expected-warning {{use of bitwise '&' with boolean operands; did you mean logical '&&'?}}
  ...
  b = foo() & bar();      // expected-warning {{use of bitwise '&' with boolean operands; did you mean logical '&&'?}}
                          // expected-note@-1 {{cast one or both operands to int to silence this warning}}
  ...
  b = foo() & !bar();     // expected-warning {{use of bitwise '&' with boolean operands; did you mean logical '&&'?}}
                          ...
                          // CHECK: fix-it:"{{.*}}":{[[@LINE-2]]:13-[[@LINE-2]]:14}:"&&"
  b = a & baz();
  b = bar() & FOO;        // expected-warning {{use of bitwise '&' with boolean operands; did you mean logical '&&'?}}
                          // expected-note@-1 {{cast one or both operands to int to silence this warning}}
                          // CHECK: fix-it:"{{.*}}":{[[@LINE-2]]:13-[[@LINE-2]]:14}:"&&"
  ...
  b = (i == 7) & foo();
#ifdef __cplusplus
  b = foo() bitand bar(); // expected-warning {{use of bitwise '&' with boolean operands; did you mean logical '&&'?}}
                          // expected-note@-1 {{cast one or both operands to int to silence this warning}}
#endif

  if (foo() & bar())      // expected-warning {{use of bitwise '&' with boolean operands; did you mean logical '&&'?}}
                          // expected-note@-1 {{cast one or both operands to int to silence this warning}}
    ;

  sink(a & b);
  sink(a & foo());
  sink(foo() & bar());    // expected-warning {{use of bitwise '&' with boolean operands; did you mean logical '&&'?}}
                          // expected-note@-1 {{cast one or both operands to int to silence this warning}}
